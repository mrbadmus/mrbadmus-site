...
⊕ **C7's seven rows added 21 Aug 2026 (MRB-272).** *Energy changes in reactions* registers
**six instrument families** in `ks3_art/c7.py` — `heating-curve`, `temp-bench`, `energy-sorter`,
`energy-uses`, `rig-plan-critique` and `rig-builder` — and every one of them is placed, so every
one of them is now something the parity gate can see and contradict.

Six families, seven rows, and the extra row is the point of the pair on `c7-01`. Design draws that
lesson's readout on `--ks3-inset` and the bar trace beside it on `--ks3-card`, so the bars sit on a
lighter ground than the panel they are read against. A later tidy-up that gave the two panels one
ground would lose that difference silently — both would still render and both would still read —
which is exactly the class of drift §10 exists to catch.

⚠️ **`rig-plan-critique` is NOT `plan-critique`.** `ks3_art/c3.py` owns `plan-critique` and the
shell class `ks3-critique-block` for c3-07, and NOTES-C7 §3 describes C7's instrument as "c3-07's
instrument with a different plan in it". That is true of the SHAPE and not of the OWNERSHIP: a
content lane may not edit another unit's module (`docs/ks3/worktrees.md` §2), so C7 registers its
own family and its own class, and MRB-279's shell-class assertion is what would have caught the
collision if it had been attempted.

⊖ **`energy-uses` is ONE family placed THREE times**, on c7-01, c7-02 and c7-03. Design draws the
same three-card judgement block on each of the first three pages, so it is one instrument used
three times rather than three that look alike — and the single row above covers it wherever it is
placed, because it is the same component.

⊖ **No new row in §10.1, and none is needed.** C7's four lessons are MODEL, PROCESS, CONTRAST and
INVESTIGATION, and all four architecture families were rowed before this unit was authored.


⊕ **C8's ten rows added 21 August 2026 (MRB-281).** *The periodic table* registers **seven
instrument families** in `ks3_art/c8.py` — `property-sorter`, `gap-filler`, `predict-cards`,
`table-reader`, `water-trough`, `halogen-grid` and `shell-strip` — and every one of them is
placed, so every one is now something the parity gate can see and contradict.

Seven families, ten rows, and the three extra rows are each the point of a pair. Both the periodic
table on `c8-03` and the halogen grid on `c8-05` draw their CELLS on `--ks3-card` and their
READOUT beside them on `--ks3-inset`, so a tapped square sits on a lighter ground than the panel
that explains it; a tidy-up giving the two one ground would lose that silently. The third is
`c8-02`'s **gap**, pinned as transparent-and-dashed rather than as a filled square, because a gap
that looked like an ordinary element would destroy the only thing that lesson is about.

⚠️ **`halogen-grid` is NOT `reactivity-grid`, and this is the third time the question has come
up.** `ks3_art/c5.py` owns `reactivity-grid` and the shell class `ks3-rgrid-block` for c5-04, and
`NOTES-C8.md` §4 describes C8's instrument as "`reactivity-grid` from C5-04 with halogens
substituted". That is true of the SHAPE and not of the OWNERSHIP: a content lane may not edit
another unit's module (`docs/ks3/worktrees.md` §2), so C8 registers its own family and its own
class — exactly as C6 did for `acid-metal-grid` and C7 for `rig-plan-critique`. MRB-279's
shell-class assertion is what would have caught the collision had it been attempted.

⊖ **`predict-cards` is ONE family placed FIVE times**, on `c8-02` `#s-rules`, `c8-03` `#s-read`,
`c8-04` `#s-predict` and `c8-06` `#s-file` and `#s-uses`. Measured off Design's own payloads all
five are `{id, q, options[], answer}`, with `#s-file`'s cards adding an optional `tag` and nothing
else. It is one instrument used five times rather than five that look alike, and the single row
above covers it wherever it is placed. `NOTES-C8.md` §4 lists `unknown-file` as a family of its
own; folding it in is a refinement inside the shape Design drew rather than a shape she did not
draw (MRB-205), and it is reported as a ruling.

⊖ **No new row in §10.1, and none is needed.** C8's six authored lessons are CONTRAST, MODEL,
MODEL, MODEL, CONTRAST and MODEL, and both architecture families were rowed long before this unit
was authored. ⚠️ Design's README labels three of them PATTERN and APPLY, **which are not among the
seven families** in architecture.md §6 and are not values `structure.py` accepts; they are mapped
by RHYTHM, and the mapping is written out in
`ks3_data/c8/lesson_04_group_1_the_alkali_metals.py`.

⊕ **C9's seven rows added 21 August 2026 (MRB-281).** *Metals and materials* registers **four
instrument families** in `ks3_art/c9.py` — `reaction-audit`, `prediction-deck`,
`extraction-route` and `spec-bench` — and every one of them is placed.

⚠️ **`reaction-audit` is NOT `reactivity-grid`, and this is the question C8's notes left open.**
`NOTES-C8.md` §4 expected C9's series to be that family's fourth use and its first past four rows;
`NOTES-C9.md` §3 declined and asked for a ruling. **Kept separate**, on three grounds in this
order: folding requires editing `ks3_art/c5.py`, which owns the name, and a lane may not edit
another unit's module; the payloads do not convert, because a `reaction-audit` cell carries an
observation, an optional equation, an optional apparatus line and a fourth *unavailable* state that
a reaction/no-reaction grid has no room for; and it would be the third metals grid in the chemistry
sequence, which is §6's adjacency warning arriving as a default. **What DID carry from C5 is the
ruling about the data** — order is `rank`, and this is the first instrument where it is tested at
six rows.

⊖ **`unit-nav` is NOT registered.** `NOTES-C9.md` §7 lists it as C9's fifth family: a four-row
`UNIT` array per page from which prev/next is derived. Design needs it because her pages are
standalone files. The engine already generates endmatter prev/next from `structure.py` for every
lesson in the key stage (§8.10 / audit law 16), so registering it would put a second source of the
same links in the build and the two would drift the first time a slot moved.

⊖ **`extraction-route` and `spec-bench` stay two families.** §7 notes both are "choose a tool, be
told what it costs you" and offers to fold them; they are kept apart on Design's own grounds. One
is a table of twenty-four AUTHORED rulings — including "works and is the wrong tool", which no tag
set produces — and the other is a computation over requirement tags. They do not convert.

⊖ **No new row in §10.1.** C9's four lessons are CLASSIFY, MODEL, PROCESS and CONTRAST, and all
four architecture families were rowed long before this unit was authored.
