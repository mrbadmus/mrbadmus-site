...
**ID scheme is not blessed yet** (§11 decision 6). IDs are generated by `statement_id()` in one place so the whole register can be reissued mechanically if Mide rules differently.
