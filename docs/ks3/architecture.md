...
- ⛔ Brand rule (superseded 2026-08-09 — see the reversal below): KS3 pages are external/public, so
  they take the **orange chevron SVG + "MrBadmusAI"** nav brand per `CLAUDE.md`, not the dashboard
  text brand.
- ⊕ **Brand rule reversed under MRB-197, ruled by Mide (2026-08-09).** KS3 takes Claude Design's
  mark: one bold `#E4572E` chevron (`M4 16L12 7l8 9`, stroke-width 4.6, round caps) plus the
  "MrBadmusAI" wordmark in Bricolage Grotesque 800 at 22px, exactly as drawn in the frozen
  reference's header. The superseded line above was this document's own initial ruling, inherited
  from `CLAUDE.md`'s external-page default before Design drew the KS3 world. Reasoning: the KS4
  gold-to-rust two-chevron mark is drawn in KS4's palette and sits as a foreign object on the KS3
  ground; the brand is part of the visual world, not an exception to it, and the key-stage split
  was already ruled for the palette (MRB-183). The KS4 mark stays mandatory on every other external
...
| 2026-07-26 | **Misconception register: `PART-12`/`PART-13` RULED to stay as they are.** Both are nature-of-science misconceptions sitting in a particles family by accident of build order, but IDs are permanent (§5.3) and both are already referenced. A candidate `NOS` family is recorded, with the decision point set **before `B10 how-we-worked-out-dna` and `C8 mendeleev` are authored**; opening it would still not move these two. ⛔ **REVERSED 17 Aug 2026 (MRB-248)** — see that row below. Kept in place, marked, per §12's reversal rule. | Claude (Opus 5) |
| 2026-08-17 | ⛔ **The 26 Jul ruling above is REVERSED, and the `NOS` family is OPEN (MRB-248, commander's ruling).** Three ids re-homed: `PART-12` → `NOS-01`, `PART-13` → `NOS-02`, `DRUG-09` → `NOS-05`. The reversal turns on a distinction the original ruling did not make — **reissuing** a number (giving `PART-12` to a new belief) silently breaks a join and stays forbidden; **re-homing** a belief and never reusing the vacated number breaks nothing silently, because every reference moves in the same change and a missed one resolves to nothing. The original ruling's own deadline is the trigger: it set the call at *before `B10 how-we-worked-out-dna` is authored* and named the cost of missing it as *"either a third home for the same idea or a rename"*. B10 is imminent. `PART-12`, `PART-13` and `DRUG-09` are **permanent gaps**; so are `GENE-06`, `ECO-12` and `REACT-18`, which were re-homed to `NOS-03`/`04`/`06` before they were minted. `ATOM-02` is cross-referenced, not moved. `DRUG-06` was named in the first cut of the list and stays in `DRUG` — it is a social-norm belief, not a claim about evidence. | Claude (Opus 5) |
| 2026-08-17 | **`elicited_by` is gated (MRB-248), by MRB-244's rule and against MRB-244's name universe.** A present value must be emitted as `id="…"` or `data-activity="…"` on the lesson's own built page. **Absence is legal**, unlike `confronted_by`: `CELL-13` and `CELL-08` document pages where a belief is stated in static markup with nothing asking the student to commit, and a gate that demanded a value would make an author invent one. Both counts reported separately. One defect found and fixed across the key stage — `ATOM-02`'s `ladder-r2`, the author's name for a rung that carries no id, corrected to `s-ladder` on the `BODY-06` precedent. | Claude (Opus 5) |
