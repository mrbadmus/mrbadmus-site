...
| 8 | `p12-04` bench · the solar-system tab's note **and its `ly` value** | *"Measured out to Neptune. Sunlight takes about four hours to cross it."*, with `ly = 0.00126` and `dia = 9e9` | *"Measured across Neptune's orbit, 9000 million km from side to side. That figure is a width and not a distance — you are inside this one — and light crosses it in hours rather than years."*, with **`ly = 0.000951`** and `dia` untouched | ⊕ **REWRITTEN 1 Sep 2026 (MRB-297). This row previously ended "Her `ly` and her `dia` are both untouched", and that is no longer true — `ly` moved.** The original departure was that her note and her readout disagreed, and 5A.1 settled it the readout's way. MRB-297's P12-09 fix then relabelled that readout from a DISTANCE to a WIDTH, which broke the assumption 5A.1 was applied under: the tab then showed **two widths of the same object, 32% apart** — 0.00126 ly is 11.0 light hours (11 900 million km) against a `dia` of 9000 million km captioned *"across its widest part"*. No rule about which side moves can reconcile two figures for one measurement, so `ly` is now the light-crossing time of `dia` itself: 9e9 ÷ 299 792.458 ÷ 3600 = 8.34 hours = 0.000951 ly. Both readouts now describe Neptune's orbit. Found by the cold double-check, not by the run that caused it. |

| 15 | `p12-04` bench · `fmtLy` below the minutes/hours cut | `if (ly < 0.001) { … ' light minutes' }` | `var HOUR_LY = 1 / (365.25 * 24); if (ly < HOUR_LY) { … ' light minutes' }` | ⊕ **1 Sep 2026 (MRB-297), and it is a consequence of row 8.** Her cut is `0.001 ly` — **8.77 light hours**, an arbitrary round number expressed in the wrong unit, so anything up to nearly nine hours printed in minutes. It never showed while the solar system sat at 0.00126; the moment row 8 corrected `ly` to 0.000951 the readout answered **"500.2 light minutes"** beside a note saying light crosses it *"in hours rather than years"*. The cut is now ONE LIGHT HOUR, the natural boundary, so each figure prints in the largest unit it fills. The Sun (1.58e-5 ly) stays in minutes; nothing else on the bench crosses the boundary. Same class as rows 6 and 7 — her VALUES are untouched and only the formatter's threshold moved. |
...
| `p12-04`'s solar-system note: *"Measured out to Neptune. Sunlight takes about four hours to cross it."* | The same tab's `ly` rendered as 11.0 light hours — about 80 AU — while her `dia` of 9 × 10^9 km is Neptune's orbit DIAMETER | ⊕ **Settled the other way in the end.** Her note was rewritten to agree with the readout (row 8, first pass); when that readout became a WIDTH the two figures could no longer both stand, and `ly` moved to match `dia`. Her note's *"out to Neptune"* is, in the end, the boundary that was kept. |
...
executor's pass. Everything it returned is accounted for above, except one:

| # | Where | What had been done | Now |
|---|---|---|---|
| R1 | `p12-05` bench note, last sentence | *"the tilt has not moved an inch"* had become *"has not moved at all"* — a paraphrase no row claimed | Hers, verbatim. (Split across two Python literals, so a grep for the phrase found nothing; the folded record did.) |

One revert. The selected-bar treatment (row 13) is also changed at
integration — see the note there.
