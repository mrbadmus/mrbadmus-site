...
Font-law sweep before the unit zip was cut. No content, layout or teaching
sequence changed — only the characters used to write formulae.

- `c2-04`, `c2-05`: every Unicode subscript is gone. In markup they are real
  `<sub>` elements (`H<sub>2</sub>O`); inside instrument payloads and rung
  strings they are plain digits (`H2SO4`, `CaCO3`, `C6H12O6`) — the same
  convention `NOTES-C4.md` records for `coefficient-balancer`.
- Why: the shipped latin subsets of Bricolage Grotesque and Instrument Sans
  carry no subscript digits, so each one was silently dropping to a system
  font mid-formula.
