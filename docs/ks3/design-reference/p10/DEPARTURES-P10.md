...
| 9 | all five benches · the pressed segmented button | the engine's shared dark rule paints it `--ks3-alert` | ⊕ **Reverted at integration, 25 Aug 2026 — the shared rule stands.** The unit had scoped `--ks3-data` to its five block classes. P11 and P12 met the same fork and kept the corpus rule (Drift 4: ONE segmented control for the system; a per-unit override is the drift that ruling exists to stop), so P10 keeps it too, and a student meets one pressed tab from B1 to P12. The corpus-wide question — amber as selection, against MRB-252 — is raised as a finding in §5, together with P11's and P12's. | (moved to *considered, not changed* in effect; the row is kept so the reasoning is not lost) |
...
executor's pass. Rows 1–19 account for every residue except one family:

| # | Where | What had been done | Now |
|---|---|---|---|
| R1–R4 | `p10-02`, `p10-03`, `p10-04`, `p10-05` — the page lede (`big_question`, under the `<h1>`) | Each had been re-authored as a question of the executor's own (*"…so what is the drawing a drawing of?"*, *"…what kind of magnet can be turned off?"*). Good hooks, not hers, and no row claimed them. | Her lede, verbatim, on all four. `p10-01`'s was already hers. |

Four reverts. Two further integration changes are recorded in rows 9 and 14
above (the pressed tab back to the shared rule; the Childline text restored
character for character in the ruled slot).
