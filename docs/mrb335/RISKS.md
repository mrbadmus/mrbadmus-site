...
| D7 | Auto composition regressing when the bank grows | `composeFromBank` takes EVERY row of the band from a lesson until `size`; more rows per lesson-band would change every auto set. | C+B: new rows take `bank_position ≥ 12` (KS4 per subtopic, KS3 per lesson); the AUTO read filters `bank_position < 12` in `bankFor` (JS) and `compose_assignment` (Python) identically; Set work reads all positions. | backend unit test `auto_ignores_positions_ge_12`; `auto_composition_unchanged` drive; `verify_questions` check 8 (pins exact id sequences) + `python3 -m ks3_data.question_bank` `_cap_test` — NOT `verify_week_truth`, which only compares week arithmetic |
