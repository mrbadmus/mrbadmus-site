"""B1 — Cells and organisation. Eight lessons, Year 7 Biology.
...
# ══════════════════════════════════════════════════════════════════════════
# L7 — Stem cells and meristems (PROCESS)
#
# beyond_statutory (§7.6). Nothing in the 2014 programme of study covers stem
# cells or meristems; `covers` is empty and `ks4_links` resolves. See FLAG 2 in
# the module docstring — this is a gap in the coverage record, not a scope
# change, and the lesson is placed after every statutory lesson so that a
# school which stops at the statutory content loses nothing.
# ══════════════════════════════════════════════════════════════════════════
{
    "slug":        "stem-cells-and-meristems",
    "title":       "Stem cells and meristems",
    "discipline":  "biology",
    "unit":        "cells-and-organisation",
    "family":      "PROCESS",

    "covers":      [],
    "touches":     ["KS3.B.CELLS.06"],
    ...
    "assumes":     [],
    "references":  [],
    "ks4_links":   ["biology/cell-biology/stem-cells"],

    "big_question": "Where do new specialised cells come from?",

    "phenomenon": {
        "kind": "demo",
        "title": "A cutting in a jar of water",
        "prompt": "One stem is snipped off a houseplant and stood in a jar of "
                  "water on the windowsill. Nothing happens for a week. Then "
                  "white roots push out of the cut end, branch, and keep "
                  "growing. Plant it and it becomes a whole new plant.",
        "commit": "There were no root cells in that stem. So where did these "
                  "come from?",
    },

    "misconceptions": [
        {"id": "CELL-09",
         "statement": "A stem cell is a cell from the stem of a plant.",
         "elicited_by": "where-does-the-word-come-from",
         "confronted_by": "stem-not-stem"},
        {"id": "CELL-10",
         "statement": "A specialised cell can change into a different kind "
                      "whenever the body needs it — a skin cell can become a "
                      "nerve cell.",
         "elicited_by": "where-from",
         "confronted_by": "once-you-choose"},
    ],

    "vocabulary": [
        {"term": "unspecialised",
         "definition": "Not yet built for any one job.",
         "note": None},
        {"term": "stem cell",
         "definition": "An unspecialised cell that can keep dividing and can "
                       "turn into other kinds of cell.",
         "note": "Nothing to do with the stem of a plant. 'Stem' here means the "
                 "thing others branch off, as in the stem of a family tree."},
        {"term": "differentiation",
         "definition": "The change from an unspecialised cell into a cell built "
                       "for one particular job.",
         "note": "Almost always a one-way trip."},
        {"term": "meristem",
         "definition": "A place in a plant where unspecialised cells are kept, "
                       "at the tips of roots and shoots.",
         "note": "A plant keeps its stem cells for its whole life. That is why a "
                 "cutting can grow an entire new root system and you cannot "
                 "regrow a finger."},
    ],

    "figures": [
        {"id": "b1-meristem-locations", "kind": "schematic",
         "caption": "A plant with the root tip and shoot tip meristems marked, "
                    "and a cutting growing new roots from the cut end.",
         "status": "needed"},
        {"id": "b1-differentiation-steps", "kind": "schematic",
         "caption": "An unspecialised cell dividing, and the new cells changing "
                    "into a root hair cell and a palisade cell.",
         "status": "needed"},
    ],

    # PROCESS rhythm (§6): lead → worked stepper with predict-gates → do-mode
    # construction of the SAME sequence (Law 5) → what follows from the
    # mechanism → ladder. The static block that would have narrated the stepper
    # is deleted, per §6.
    "core": [
        {"type": "hook", "ref": "phenomenon"},
        {"type": "check", "id": "where-from"},
        {"type": "explainer", "id": "cells-that-never-chose",
         "text": "From cells that never specialised. Most cells pick a job early "
                 "and keep it — a root hair cell will be a root hair cell for as "
                 "long as it lives. But a plant holds back a small group of "
                 "cells that never choose. They stay unspecialised, they can "
                 "still divide, and they can still become anything the plant "
                 "needs. Cut the stem and those are the cells that go to work.",
        },
        {"type": "misconception", "id": "once-you-choose", "targets": "CELL-10"},
        {"type": "check", "id": "where-does-the-word-come-from"},
        {"type": "misconception", "id": "stem-not-stem", "targets": "CELL-09"},
        {"type": "keyword", "terms": ["unspecialised", "stem cell",
                                      "differentiation", "meristem"]},
        {"type": "worked-example", "id": "rooting-stepper"},
        {"type": "check", "id": "rooting-do"},
        {"type": "figure", "ref": "b1-differentiation-steps"},
        {"type": "practical", "id": "tip-test"},
        {"type": "figure", "ref": "b1-meristem-locations"},
        {"type": "explainer", "id": "what-this-buys",
         "text": "So a plant keeps a supply of unspecialised cells at every "
                 "growing tip, for life. That is why you can take ten cuttings "
                 "off one plant and get ten plants. You keep far fewer, in a few "
                 "places — your bone marrow makes new blood cells every day, "
                 "which is why you recover from a cut. But you have no meristem, "
                 "and no store of cells waiting to become a finger, so a lost "
                 "finger does not grow back."},
        {"type": "check", "id": "why-plants-not-people"},
        {"type": "quiz", "ref": "ladder"},
        {"type": "summary", "ref": "key_note"},
    ],

    "stretch": [
        {"type": "explainer", "id": "stem-cells-in-medicine",
         "text": "Doctors already use this. In a bone marrow transplant, "
                 "unspecialised cells from a healthy donor are put into a "
                 "patient whose own marrow has stopped making healthy blood "
                 "cells. If they settle in, they divide and differentiate, and "
                 "the patient starts making their own blood cells again. It "
                 "works for the same reason the cutting does: the cells had not "
                 "yet chosen a job."},
        {"type": "check", "id": "transplant-explain"},
    ],

    "support": [],

    "activities": [
        {"id": "where-from", "kind": "predict",
         "demand": "elicit CELL-10 by forcing a commitment on the hook",
         "prompt": "Commit before you read on. Where did the root cells on that "
                   "cutting come from?",
         "options": ["Stem cells in the stem changed into root cells",
                     "Some cells in the stem had never specialised at all",
                     "The roots grew from cells that were already in the water"],
         "answer": 1,
         "reveal": "From cells that had never specialised. Nothing turned back "
                   "into anything — the next block explains why that matters.",
         "targets": "CELL-10"},
        {"id": "once-you-choose", "kind": "confrontation",
         "demand": "make the wrongness of CELL-10 visible",
         "prompt": "The wrong idea, in the words students really write: 'the "
                   "stem cells changed into root cells.' Test it on yourself. If "
                   "a specialised cell could switch jobs whenever the body "
                   "needed it, a skin cell beside a cut would simply become "
                   "whatever was missing, and nothing would ever scar.",
         "reveal": "Scars are the evidence. A specialised cell has committed: it "
                   "keeps its shape and its job and it does not go back. What "
                   "the cutting used was a small group of cells that had never "
                   "committed in the first place — unspecialised, still able to "
                   "divide, still able to become anything. Differentiation is a "
                   "one-way trip, so an organism has to keep some cells that "
                   "have not taken it.",
         "targets": "CELL-10"},
        {"id": "where-does-the-word-come-from", "kind": "predict",
         "demand": "elicit CELL-09 — a word trap, and Law 7's job",
         "prompt": "You have met the name. Commit to what it means: why are they "
                   "called stem cells?",
         "options": ["Because they were first found in the stems of plants",
                     "Because everything else stems from them, like a family "
                     "tree",
                     "Because they are shaped like a stem, with a long thin "
                     "body"],
         "answer": 1,
         "reveal": "Because everything else stems from them. Animals have them "
                   "too, and animals have no stems.",
         "targets": "CELL-09"},
        {"id": "stem-not-stem", "kind": "confrontation",
         "demand": "make the wrongness of CELL-09 visible",
         "prompt": "Say the wrong idea plainly: 'a stem cell is a cell from a "
                   "plant's stem.' Now try to use that sentence about a human "
                   "bone marrow transplant, where stem cells are taken out of a "
                   "person's hip.",
         "reveal": "The sentence falls apart, because people do not have stems. "
                   "'Stem' is being used the way it is in a family tree — the "
                   "thing all the branches come off. A stem cell is any "
                   "unspecialised cell that others come from, in a plant or an "
                   "animal. In plants they happen to sit at the tips of roots "
                   "and shoots, in places called meristems, which is where the "
                   "confusion starts.",
         "targets": "CELL-09"},
        {"id": "rooting-stepper", "kind": "worked-example",
         "demand": "model the mechanism in steps before asking for it (Law 5)",
         "prompt": "Watch this one set out step by step, because you build the "
                   "next one. How a cutting grows roots. Step 1: the stem is cut "
                   "just below the point where a leaf joins it, where "
                   "unspecialised cells are. Step 2: those cells start dividing, "
                   "making more cells just like themselves. Step 3: the new "
                   "cells differentiate — they change shape and contents to "
                   "become root cells, including root hair cells with long thin "
                   "hairs. Step 4: the new root takes in water, and the cutting "
                   "can keep itself alive.",
         "reveal": "Four steps, and the order matters. Divide first, "
                   "differentiate second. A cell cannot specialise and then "
                   "divide into more of itself, because specialising is the step "
                   "that ends the choosing."},
        {"id": "rooting-do", "kind": "construct",
         "demand": "the same artifact, produced by the student (Law 5)",
         "prompt": "Your turn, same four steps, different plant. A spider plant "
                   "sends out a long runner with a tiny plantlet on the end of "
                   "it, and that plantlet grows its own roots. Set out the four "
                   "steps that produce those roots.",
         "success": ["Step 1 names unspecialised cells in the plantlet",
                     "Step 2 says they divide",
                     "Step 3 says the new cells differentiate into root cells",
                     "Step 4 says the roots then take in water",
                     "Divide comes before differentiate"]},
        {"id": "tip-test", "kind": "investigation",
         "demand": "investigate — locate the meristem by breaking it",
         "prompt": "Two identical seedlings, same pot, same light. The very tip "
                   "of the shoot is snipped off the first. A leaf halfway down "
                   "the stem is removed from the second. Predict which one stops "
                   "getting taller, write your prediction down first, then say "
                   "what result would prove you wrong.",
         "success": ["Predicts the seedling with the tip removed stops growing "
                     "taller",
                     "Says the meristem, where new cells are made, is at the tip",
                     "Names what is kept the same: pot, light, water, size of "
                     "seedling",
                     "States a result that would disprove the prediction",
                     "Writes the prediction before the result, not after"]},
        {"id": "why-plants-not-people", "kind": "construct",
         "demand": "explain the consequence of the mechanism",
         "prompt": "Now use the mechanism on the obvious question. A gardener "
                   "takes ten cuttings from one plant and every one grows into a "
                   "new plant. Explain why that works for a plant, and why you "
                   "cannot regrow a lost finger.",
         "success": ["Says plants keep unspecialised cells in meristems for life",
                     "Says those cells can divide and differentiate into "
                     "whatever is needed",
                     "Says humans keep far fewer, in a few places only",
                     "Says a specialised cell cannot change back"]},
        {"id": "transplant-explain", "kind": "construct",
         "demand": "transfer the mechanism to a medical case",
         "prompt": "Same mechanism, a hospital instead of a windowsill. Explain "
                   "why a bone marrow transplant can let a patient start making "
                   "their own blood cells again.",
         "success": ["Says the donated cells are unspecialised",
                     "Says they divide once they settle in",
                     "Says they differentiate into blood cells",
                     "Says a specialised blood cell could not have done this"]},
    ],

    "ladder": {
        "recall": {
            "q": "What is a stem cell?",
            "options": ["An unspecialised cell that can become other kinds",
                        "A cell taken from the stem of a plant",
                        "A cell that has lost its nucleus to make room",
                        "A cell that has finished dividing for good"],
            "answer": 0,
            "feedback": {
                1: "Animals have stem cells and no stems. The word means the "
                   "thing others branch off. (CELL-09)",
                2: "That is a red blood cell, and it is fully specialised. "
                   "(CELL-05)",
                3: "The opposite — being able to keep dividing is the point of "
                   "them.",
            }},
        "apply": {
            "q": "The very tip of a shoot is snipped off. The plant stays alive "
                 "and green but stops getting taller. Why?",
            "options": ["The meristem, where new cells are made, has been "
                        "removed",
                        "The plant has lost too many leaves to make its food",
                        "Water escapes from the cut, so the stem cannot grow",
                        "The roots stop working when the shoot is damaged"],
            "answer": 0,
            "feedback": {
                1: "Only the tip was taken; every leaf is still there and still "
                   "green.",
                2: "A cut stem seals over. Cuttings grow roots in open water.",
                3: "The roots are untouched, and the plant is still alive and "
                   "taking in water.",
            }},
        "explain": {
            "q": "Explain how one unspecialised cell in a cutting ends up as a "
                 "root hair cell.",
            "success": ["Says the unspecialised cell divides first",
                        "Says the new cells then differentiate",
                        "Says differentiating means changing shape and contents "
                        "to fit a job",
                        "Names the finished feature — the long thin hair"]},
        "produce": {
            "q": "A gardener takes ten cuttings from one plant and every one "
                 "grows roots. Explain why this works with plants, and why you "
                 "cannot regrow a lost finger.",
            "success": ["Says plants keep unspecialised cells at their tips for "
                        "life",
                        "Says those cells divide and differentiate into what is "
                        "needed",
                        "Says humans keep very few, in a few places such as bone "
                        "marrow",
                        "Says specialised cells cannot change back"]},
    },

    "key_note": "Most cells specialise and stay that way — differentiation is a "
                "one-way trip. Unspecialised cells, called stem cells, can still "
                "divide and still become something else. Plants keep theirs for "
                "life in meristems at root and shoot tips, which is why a "
                "cutting grows roots. A stem cell has nothing to do with a "
                "plant's stem.",
...
# ══════════════════════════════════════════════════════════════════════════
# L8 — Enzymes and what changes their rate (MODEL)
#
# beyond_statutory (§7.6). Enzymes appear in the programme of study once, as a
# parenthesis inside KS3.B.NUT.04 — "(enzymes simply as biological catalysts)" —
# which B3 owns and needs. §7.6 already assumes this lesson exists: XB1's bridge
# lesson is justified by "B1 teaches enzyme rate descriptively". See FLAG 2.
# ══════════════════════════════════════════════════════════════════════════
{
    "slug":        "enzymes-and-rate",
    "title":       "Enzymes and what changes their rate",
    "discipline":  "biology",
    "unit":        "cells-and-organisation",
    "family":      "MODEL",

    "covers":      [],
    "touches":     ["KS3.B.NUT.04"],
    "beyond_statutory": True,
    "threads":     [{"id": "cells-and-systems", "level": 3},
    ...
    "assumes":     [],
    "references":  [],
    "ks4_links":   ["biology/organisation/enzymes"],

    "big_question": "What makes the reactions inside a cell fast enough to keep "
                    "you alive?",

    "phenomenon": {
        "kind": "demo",
        "title": "Two tubes, one piece of liver each",
        "prompt": "Two test tubes of hydrogen peroxide. A cube of raw liver goes "
                  "into the first and it erupts into foam in about two seconds. "
                  "A cube of the same liver, boiled and cooled back down, goes "
                  "into the second. Nothing happens at all. Nothing was taken "
                  "out of it — it was only in hot water.",
        "commit": "Boiling removed nothing. So what did it do?",
    },

    "misconceptions": [
        {"id": "CELL-11",
         "statement": "The enzymes were killed by the heat.",
         "elicited_by": "what-did-boiling-do",
         "confronted_by": "not-killed-changed"},
        {"id": "CELL-12",
         "statement": "The hotter it is the faster any reaction goes, so an "
                      "enzyme works best as hot as possible.",
         "elicited_by": "predict-the-curve",
         "confronted_by": "two-halves-of-the-curve"},
    ],

    "vocabulary": [
        {"term": "enzyme",
         "definition": "A substance made by cells that speeds up a reaction "
                       "without being used up itself.",
         "note": "One cell makes thousands of different ones, each doing one "
                 "job."},
        {"term": "catalyst",
         "definition": "Anything that speeds up a reaction without being used "
                       "up.",
         "note": "Enzymes are the catalysts that living things make. You will "
                 "meet others in chemistry."},
        {"term": "denatured",
         "definition": "Changed shape by heat or by the wrong conditions, so it "
                       "no longer works.",
         "note": "An enzyme is not alive, so it cannot die and it cannot be "
                 "killed. The word is denatured, and the change does not undo "
                 "when it cools."},
        {"term": "optimum",
         "definition": "The conditions in which something works best.",
         "note": "Most enzymes in your body have an optimum near 37 °C, which is "
                 "body temperature. That is not a coincidence."},
    ],

    "figures": [
        {"id": "b1-liver-peroxide", "kind": "apparatus",
         "caption": "Two test tubes of hydrogen peroxide side by side, one with "
                    "raw liver foaming over the top, one with boiled liver "
                    "unchanged.",
         "status": "needed"},
        {"id": "b1-enzyme-rate-curve", "kind": "graph",
         "caption": "Rate of reaction against temperature for an enzyme: rising "
                    "to a peak near 37 °C, then falling steeply to zero.",
         "status": "needed"},
        {"id": "b1-enzyme-shape", "kind": "schematic",
         "caption": "An enzyme and the substance it works on, before heating and "
                    "after, showing that the enzyme's shape no longer fits.",
         "status": "needed"},
    ],

    # MODEL rhythm (§6), with one honest departure recorded in FLAG 6: the
    # flagship is a declared graph plus a Law 5 read-the-curve pair rather than
    # a parameter instrument, because no `rate-curve` component exists and two
    # blocking new components is already the limit for one unit.
    "core": [
        {"type": "hook", "ref": "phenomenon"},
        {"type": "check", "id": "what-did-boiling-do"},
        {"type": "explainer", "id": "changed-not-killed",
         "text": "It changed a shape. Liver is full of an enzyme that breaks "
                 "hydrogen peroxide down into water and oxygen — the foam is the "
                 "oxygen. An enzyme works because its shape fits the substance "
                 "it works on, the way a key fits a lock. Heat shakes that shape "
                 "apart, and it does not go back when it cools. The boiled liver "
                 "still contains the enzyme. It is just the wrong shape now, and "
                 "the wrong shape does nothing."},
        {"type": "figure", "ref": "b1-liver-peroxide"},
        {"type": "misconception", "id": "not-killed-changed", "targets": "CELL-11"},
        {"type": "figure", "ref": "b1-enzyme-shape"},
        {"type": "keyword", "terms": ["enzyme", "catalyst", "denatured",
                                      "optimum"]},
        {"type": "check", "id": "predict-the-curve"},
        {"type": "figure", "ref": "b1-enzyme-rate-curve"},
        {"type": "misconception", "id": "two-halves-of-the-curve",
         "targets": "CELL-12"},
        {"type": "worked-example", "id": "read-the-curve"},
        {"type": "check", "id": "read-the-curve-do"},
        {"type": "practical", "id": "washing-powder-test"},
        {"type": "check", "id": "washing-powder-explain"},
        {"type": "quiz", "ref": "ladder"},
        {"type": "summary", "ref": "key_note"},
    ],

    "stretch": [
        {"type": "explainer", "id": "enzymes-everywhere",
         "text": "Every cell in you is running thousands of these at once, each "
                 "shaped for one job: one to break down sugar, another to build "
                 "a protein, another to copy instructions. Without them the "
                 "reactions that keep you alive would still happen — just far, "
                 "far too slowly to keep you warm, moving or growing. A cell is "
                 "less like a bag of chemicals than a workshop full of tools."},
        {"type": "check", "id": "why-so-many"},
    ],

    "support": [],

    "activities": [
        {"id": "what-did-boiling-do", "kind": "predict",
         "demand": "elicit CELL-11 by forcing a commitment on the hook",
         "prompt": "Commit before you read on. Nothing was removed from the "
                   "boiled liver. So what did the boiling do?",
         "options": ["It killed the enzymes in the liver",
                     "It changed the shape of the enzymes permanently",
                     "It used the enzymes up, so there are none left"],
         "answer": 1,
         "reveal": "It changed their shape, and the change is permanent. The "
                   "word 'killed' is the one to lose, and the next block says "
                   "why it matters.",
         "targets": "CELL-11"},
        {"id": "not-killed-changed", "kind": "confrontation",
         "demand": "make the wrongness of CELL-11 visible",
         "prompt": "The idea to kill, in the words nearly everybody writes: 'the "
                   "heat killed the enzymes.' Ask one question of it. To be "
                   "killed, a thing has to have been alive. Score an enzyme on "
                   "the seven life processes from lesson one — does it move, "
                   "feed, sense, grow, reproduce, excrete or respire?",
         "reveal": "None of the seven. An enzyme is a substance, not an "
                   "organism; it is made by a living cell but it is no more "
                   "alive than the sugar in your tea. So it cannot be killed. "
                   "What actually happened is that its shape was shaken apart "
                   "and did not go back — the word for that is denatured. Same "
                   "enzyme, wrong shape, no reaction.",
         "targets": "CELL-11"},
        {"id": "predict-the-curve", "kind": "predict",
         "demand": "elicit CELL-12",
         "prompt": "So cold liver is slow and boiled liver does nothing. Commit "
                   "to the shape of the whole graph before you look at it: what "
                   "happens to the rate as the temperature goes up from 0 °C to "
                   "80 °C?",
         "options": ["It rises the whole way — hotter is always faster",
                     "It rises to a peak, then falls steeply to nothing",
                     "It stays flat until about 37 °C and then stops dead"],
         "answer": 1,
         "reveal": "Up to a peak, then off a cliff. The peak is at about 37 °C, "
                   "and both halves of that shape have a reason.",
         "targets": "CELL-12"},
        {"id": "two-halves-of-the-curve", "kind": "confrontation",
         "demand": "make the wrongness of CELL-12 visible",
         "prompt": "Say the wrong idea out loud: 'hotter is always faster, so an "
                   "enzyme works best as hot as you can get it.' Half of that is "
                   "true, which is exactly why it survives. Look at the two "
                   "halves of the curve separately.",
         "reveal": "Below the peak, hotter really is faster: everything is "
                   "moving quicker, so the enzyme and the substance meet more "
                   "often. Past the peak the enzyme's shape starts coming apart, "
                   "and a shape that no longer fits cannot work however fast it "
                   "is moving. So the rate collapses. 'Hotter is faster' is the "
                   "left-hand half of a curve, mistaken for the whole of it — "
                   "and it is why your body holds itself at 37 °C rather than "
                   "running as hot as it can.",
         "targets": "CELL-12"},
        {"id": "read-the-curve", "kind": "worked-example",
         "demand": "model the reading before asking for it (Law 5)",
         "prompt": "Watch this graph read properly, because you read the next "
                   "one. Take the enzyme curve above. Step 1: name the axes — "
                   "temperature across, rate of reaction up. Step 2: describe "
                   "the first part — the rate rises steadily from 0 °C to about "
                   "37 °C. Step 3: find the peak and read its value off the "
                   "temperature axis — 37 °C, the optimum. Step 4: describe what "
                   "happens after it — the rate falls steeply and reaches zero "
                   "by about 60 °C, because the enzyme is denatured.",
         "reveal": "Four steps: axes, first part, peak, last part. Do those four "
                   "in that order on any graph like this one and you will not "
                   "miss anything."},
        {"id": "read-the-curve-do", "kind": "construct",
         "demand": "the same artifact, produced by the student (Law 5)",
         "prompt": "Your turn, same four steps, different graph. An enzyme in "
                   "your stomach is tested at pH 1 to pH 10. Its rate rises from "
                   "pH 1, peaks at pH 2, and falls to nothing by pH 6. Describe "
                   "that graph using the same four steps, and say what the "
                   "optimum is.",
         "success": ["Names the axes: pH across, rate up",
                     "Describes the rise before the peak",
                     "States the optimum as pH 2",
                     "Describes the fall to zero after it",
                     "Says the enzyme is denatured by the wrong conditions"]},
        {"id": "washing-powder-test", "kind": "investigation",
         "demand": "investigate — judge a method, then fix it",
         "prompt": "A student tests a biological washing powder on identical "
                   "grass-stained cloths. They wash one at 20 °C for ten "
                   "minutes, one at 40 °C for twenty minutes, and one at 90 °C "
                   "for five minutes, then rank the cloths by eye. Find "
                   "everything wrong with this method and write the version you "
                   "would run.",
         "success": ["Spots that the washing time is not kept the same",
                     "Names at least two other things to keep the same, such as "
                     "the amount of powder and the size of the stain",
                     "Says ranking by eye is a judgement, and suggests something "
                     "measurable",
                     "Keeps temperature as the one thing that changes",
                     "Says the test should be repeated"]},
        {"id": "washing-powder-explain", "kind": "construct",
         "demand": "explain an everyday instruction with the model",
         "prompt": "Now explain the label. A biological washing powder says "
                   "'wash at 40 °C'. Say what happens to the stains at 20 °C and "
                   "at 90 °C, and why the instruction is what it is.",
         "success": ["At 20 °C the enzymes work, but slowly",
                     "At 90 °C the enzymes are denatured and stop working",
                     "Says 40 °C is close to the optimum",
                     "Does not say the enzymes are killed at 90 °C"]},
        {"id": "why-so-many", "kind": "construct",
         "demand": "transfer the shape idea to explain a number",
         "prompt": "Use the shape idea to explain a number. One cell makes "
                   "thousands of different enzymes rather than one all-purpose "
                   "one. Explain why it has to.",
         "success": ["Says an enzyme works because its shape fits one substance",
                     "Says a shape that fits one thing will not fit another",
                     "Says each reaction therefore needs its own enzyme",
                     "Says the enzyme is not used up, so one of each is enough "
                     "to keep working"]},
    ],
    ...
    "ladder": {
        "recall": {
            "q": "What is an enzyme?",
            "options": ["A substance made by cells that speeds up a reaction",
                        "A type of cell found in the liver and the stomach",
                        "The gas given off when liver meets hydrogen peroxide",
                        "A chemical that is used up as the reaction happens"],
            "answer": 0,
            "feedback": {
                1: "An enzyme is a substance a cell makes, not a kind of cell.",
                2: "That is oxygen — the foam, not the cause of it.",
                3: "A catalyst is not used up. That is what makes it a catalyst.",
            }},
        "apply": {
            "q": "An enzyme works fastest at 37 °C. A student heats it to 70 °C "
                 "and the reaction stops completely. What has happened?",
            "options": ["The enzyme has been denatured — its shape has changed",
                        "The enzyme has been killed by the high temperature",
                        "The enzyme has been used up by the reaction",
                        "The enzyme has dissolved into the liquid around it"],
            "answer": 0,
            "feedback": {
                1: "An enzyme was never alive, so nothing can kill it. Its shape "
                   "changed. (CELL-11)",
                2: "A catalyst is not used up — it can work over and over.",
                3: "Dissolving would not stop it working, and it is not what "
                   "heat does to it.",
            }},
        "explain": {
            "q": "Explain the shape of a graph of enzyme rate against "
                 "temperature — both halves of it.",
            "success": ["Says the rate rises at first because everything moves "
                        "faster and meets more often",
                        "Says there is a peak, at the optimum",
                        "Says the rate then falls because the enzyme's shape "
                        "changes",
                        "Uses the word denatured, and does not say killed"]},
        "produce": {
            "q": "A biological washing powder says 'wash at 40 °C'. Explain what "
                 "happens to the stains at 20 °C and at 90 °C, and why the "
                 "instruction says 40 °C.",
            "success": ["At 20 °C the enzymes still work but more slowly",
                        "At 90 °C they are denatured and stop working",
                        "Says the change at 90 °C does not undo on cooling",
                        "Says 40 °C is near the optimum, so the stains come out "
                        "fastest"]},
    },

    "key_note": "An enzyme is a catalyst that a cell makes: it speeds a reaction "
                "up and is not used up. Warming speeds it up as far as its "
                "optimum, near 37 °C in your body. Past that its shape comes "
                "apart and does not go back — denatured, never killed — and it "
