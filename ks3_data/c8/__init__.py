...
═══════════════════════════════════════════════════════════════════════════
SEVEN SLOTS, SIX AUTHORED — AND THE SEVENTH IS NOT A GAP TO APOLOGISE FOR
═══════════════════════════════════════════════════════════════════════════

`ks3_data/structure.py` carries SEVEN C8 slots since MRB-281. Six are authored
here. The seventh — `metal-and-non-metal-oxides`, which owns `KS3.C.PT.06` —
is not, and renders as an honest coming-soon page (§11 decision 8).

⚠️ **`KS3.C.PT.06` IS THE ONLY UNCOVERED STATUTORY STATEMENT IN C1–C8.** That
is a real hole in the curriculum and it is recorded as one, in the unit
wrapper, in `NOTES-C8.md` §1 and here. It is not fixed by this run.
...
which does not exist. MRB-257 ruled that a placeholder is the build's backlog
printed to a twelve-year-old, and a sentence pointing at a lesson that is not
there is exactly that. The unit index already carries unbuilt slots as
coming-soon rows; that convention needs no help from prose and it resolves on
its own the day c8-07 ships. c8-01's BACK link to `c7-04` is untouched — that
page exists.
...
ENER.02a–c pattern:
    ...
    c8-07 metal-and-non-metal-oxides PT.06   ← NOT AUTHORED
...
def lessons():
    ...
    apologised for. `metal-and-non-metal-oxides` is that slot today.
    """
    found = []
