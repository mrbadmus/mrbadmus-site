...
(function() {
  ...
  fetch(RENDER_URL + '/api/weekly-leaderboard/landing').then(function(r){return r.ok?r.json():null;}).then(function(data){
    if (!data) return;
    var hasAny = (data.foundation_top3 && data.foundation_top3.length) || (data.higher_top3 && data.higher_top3.length) || data.champion;
    if (!hasAny) return;
    document.getElementById('top-stars').style.display = 'block';
    document.getElementById('foundation-stars').innerHTML = renderStars(data.foundation_top3, 'foundation');
    document.getElementById('higher-stars').innerHTML = renderStars(data.higher_top3, 'higher');
    if (data.champion) {
      var s = data.champion;
      ...
      document.getElementById('sotw-score').textContent = s.percentage + '%';
      document.getElementById('sotw-subjects').textContent = s.score + '/' + s.max_score + ' across all 3 subjects';
...
(function() {
  ...
  const INDEX = [
    ...
    {t:'Using the Earth\'s Resources',s:'Chemistry',url:'/combined/higher/chemistry/using-resources/earths-resources.html',c:'#FFD2E6'},
    {t:'Potable Water and Water Treatment',s:'Chemistry',url:'/combined/higher/chemistry/using-resources/potable-water.html',c:'#FFD2E6'},
    {t:'Life Cycle Assessment',s:'Chemistry',url:'/combined/higher/chemistry/using-resources/life-cycle-assessment.html',c:'#FFD2E6'},
    {t:'Reducing Use of Resources',s:'Chemistry',url:'/combined/higher/chemistry/using-resources/reducing-use-of-resources.html',c:'#FFD2E6'},
    ...
    {t:'Alternative Methods of Extracting Metals',s:'Chemistry',url:'/combined/higher/chemistry/using-resources/alternative-metal-extraction.html',c:'#FFD2E6'},
