...
GATES = [
    ...
    dict(name="ks4_pool_drive",
         ...
         why="MRB-332, ported to Set work v2 at MRB-335 — the same rule, "
             ...
             "rule and a query that forgets to apply it look identical from "
             "the outside, so this signs a throwaway teacher in for real, "
             "against the real TEST project through a locally-run backend "
             "(`MRB_BACKEND`, port 5532), and reads what actually comes back "
             "from `/api/teacher/set-work/scope` and `/preview`. Nothing is "
             "stubbed and no check runs on the service key — a proof carried "
             "on service role proves nothing about a child, because service "
             "role bypasses RLS. "
             "⚠️ THE TIER AND THE PATHWAY COME FROM DIFFERENT PLACES, AND "
             "THAT SPLIT IS WHAT THIS GATE NOW WATCHES. Set work's tier is "
             "the REQUEST's — a teacher chooses it per set — so a Foundation "
             "class may legitimately be set Higher extension and a Higher "
             "class Foundation revision, and the drive asserts BOTH answer "
             "200 and draw from different pools. Set work's pathway is always "
             "the CLASS's and no route accepts it as a parameter; that "
             "asymmetry is the only reason `triple_only` is unreachable on a "
             "combined class, and it is sealed at the SCOPE (the tree is "
             "re-derived from the class row, so a foreign slug is 400 "
             "`scope_not_for_class` before a bank row is read) rather than at "
             "the pool, which is stronger because one function guards all "
             "four read paths and a deliberate refusal is evidence where "
             "'0 rows served' is not. ⚠️ v1's opposite assertion — 'a "
             "Foundation class is never served a Higher question' — was true "
             "when the class row carried the tier and is now the defect; the "
             "class tier survives only as `defaultTierFor`'s opening chip, "
             "and as the AUTOMATIC producer's own read, which is a different "
             "question and also stops at `bank_position < 12` where Set work "
             "reads every position. "
             "⚠️ THE COLLISION CHECK IS STILL THE ONE TO READ BEFORE EDITING. "
             "Eight KS4 subtopic slugs are byte-identical to KS3 lesson slugs "
             "(catalysts, chromatography, distance-time-graphs and five "
             "more). Each holds twelve KS3 bank rows AND a full set of KS4 "
             "pool rows, so a COUNT is true whether the route read the right "
             "table or the wrong one — asserting the number would produce a "
             "check that passes on the bug it exists to catch, which is worse "
             "than no check because it reads as coverage. What separates the "
             "pools is row IDENTITY: a KS4 id begins `ks4-`, a KS3 id looks "
             "like `c1-04-h02`. So it asserts every id in a KS4 payload "
             "carries the KS4 prefix and that no KS3 id appears at all. "
             "⊕ MRB-335 also fixed WHICH class each of the eight is driven "
             "against. v1 drove all eight at 10a/Bi1; that class is triple "
             "BIOLOGY, and v2 scopes on the class's own tree, so seven "
             "chemistry and physics slugs would have answered "
             "`scope_not_for_class` and the check would have gone green "
             "having measured one of eight. Each now goes to the class whose "
             "tree holds it — six on the combined class, `electric-fields` "
             "(triple-only physics) on 10c/Ph1, the fixture's only class that "
             "can reach it. "
             "SKIPPED until mrb331_fixture.py exists: it imports MRB-331's "
             "fixture and needs its three KS4 classes, so before the rebase "
             "there is nothing coherent for it to press. That is a named "
             "skip, not silence — prepush_gate reports it."),
    ...
    dict(name="student_bell_drive",
         ...
         why="MRB-337 — THE BELL ON THE STUDENT PAGES, AND EVERY WAY IT CAN "
             ...
             "⊕ MRB-337/N5, 8 Sep 2026 — AND A WIDTH LADDER, because 390 and "
             "1280 were not enough. The bell overflowed the bar at 390px and "
             "ALSO at 414px, by very much more (501-508px against 390's "
             "427-434px), and the 414 case went unseen for a day because a "
             "defect that lives strictly BETWEEN the two widths a gate "
             "measures is invisible to that gate for ever. ⚠️ 414 is the "
             "WORST width, not 390: nav.css collapses Leaderboard and Search "
             "into the drawer at <=400px, so the bar carries two more items "
             "just above that hinge than it does on the smallest phone, and "
             "'test the smallest phone' is the intuition that produced the "
             "blind spot. `phase_ladder` therefore asks at 360, 375, 390, "
             "393, 400, 414 and 430 on the four signed-in pages that failed, "
             "and asks TWO questions per cell, both as DIFFERENCES so "
             "neither can go red for somebody else's pre-existing overflow: "
             "what does the bell cost (measure, remove it, measure again), "
             "and what does a 32-CHARACTER DISPLAY NAME cost (measure, "
             "lengthen it, measure again). The second is the guard on "
             "`.nav-chip-name`; a short fixture name would pass whatever the "
             "CSS said. `chip_is_never_the_email` pins the other half — "
             "shared/nav.js used to fall back to the email local part, which "
             "is why TEST drew `mrb326_pupil_a` where production drew a real "
             "first name, and why the defect was louder in the sandbox than "
             "in the world. "
