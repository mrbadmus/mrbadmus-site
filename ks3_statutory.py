...
--------------------------------------------------
architecture.md §11 decision 6 (the ``KS3.C.PNM.02`` ID scheme) has **not been
blessed yet**. IDs must therefore be **mechanically reissuable**: if Mide rules
differently, edit ``STRAND_CODES`` / the ordinal rule below and re-run. Nothing
is hand-scattered in prose.
...
def render():
    ...
    w("**ID scheme is not blessed yet** (§11 decision 6). IDs are generated by "
      "`statement_id()` in one place so the whole register can be reissued "
      "mechanically if Mide rules differently.\n\n")
    w("---\n\n")
