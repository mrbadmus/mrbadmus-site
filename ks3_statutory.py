...
is revised, append new IDs and mark superseded ones; never reuse.
    ...
    python3 ks3_statutory.py            # verify, write nothing (the default)
    python3 ks3_statutory.py --write    # regenerate the register

⚠️ A BARE RUN NO LONGER WRITES. It used to, and that is how MRB-232's ruling
was destroyed by a run that reported PASS — see the `RULINGS` note further
down. `--check-only` is what `gate_registry.py` runs.
...
# ── HAND-RULED ADDITIONS TO A GENERATED FILE (MRB-282, 23 Aug 2026) ─────
#
# ⚠️ THIS IS WHY THE REGISTER IS NOT REGENERATED BY A BARE RUN ANY MORE.
#
# `docs/ks3/statutory-register.md` says "Generated file — do not hand-edit",
# and it was hand-edited anyway, correctly and on purpose: MRB-232's ruling
# that `KS3.B.NUT.02` splits between B3 and P2 was written INTO the markdown,
# as an overridden owner cell plus a prose block under the NUT table.
#
# The next `python3 ks3_statutory.py` rewrote the file whole and deleted both.
# It printed "verified 155 statements" and exited 0 while doing it, because
# `verify()` checks the TRANSCRIPTION and the ID space and has never had any
# opinion about what is in the file it is about to overwrite. The loss was
# recovered by hand from git, and would not have been noticed at all had
# somebody not diffed a tracked file after a green run.
#
# A gate that mutates the source of truth it is checking is not a gate. So:
#
#   1. a bare run is now `--check-only` and writes nothing;
#   2. `--write` REFUSES to delete content it cannot reproduce;
#   3. a ruling that must survive regeneration lives HERE, in the module that
#      is the single source of truth, and is emitted by `render()` — which
#      makes losing it impossible rather than merely rude.
#
# Adding a ruling: append a dict. `id` anchors it to a statement (the block is
# emitted after that statement's strand table, and `owner` replaces that row's
# proposed-owner cell). `block` is verbatim markdown.
RULINGS = [
    dict(
        id="KS3.B.NUT.02",
        owner="**B3 + P2, split — see below**",
        block="""##### `KS3.B.NUT.02` is SPLIT between B3 and P2 — ruled 16 Aug 2026 (MRB-232)
...
def ruling_for(statement_id_):
    """The ruling anchored to a statement, or None."""
    for r in RULINGS:
        if r["id"] == statement_id_:
            return r
    return None
...
def _rows(entries, with_owner):
    ...
    for s in entries:
        text = s["text"].replace("|", "\\|")
        if with_owner:
            ruled = ruling_for(s["id"])
            owner = ((ruled and ruled.get("owner"))
                     or s["proposed_unit"] or "—")
            out.append("| `%s` | %s | %s |\n" % (s["id"], text, owner))
        else:
            out.append("| `%s` | %s |\n" % (s["id"], text))
    return "".join(out)
...
def render():
    ...
    w("**Generated file — do not hand-edit.** Source of truth is `ks3_statutory.py` "
      "at the repo root; regenerate with `python3 ks3_statutory.py --write`. "
      "A bare run and `--check-only` verify without writing \u2014 see the "
      "`RULINGS` note in that file for why.\n\n")
    ...
    for d in ("B", "C", "P"):
        ...
        for group in STRANDS:
            ...
            if group["strand"] != group["area"]:
                w("#### %s\n\n" % group["strand"])
            w("Strand code `%s`.\n\n" % group["code"])
            here = [s for s in subject if s["discipline"] == d
                    and s["code"] == group["code"]]
            w(_rows(here, True))
            w("\n")
            for s in here:
                ruled = ruling_for(s["id"])
                if ruled and ruled.get("block"):
                    w(ruled["block"].rstrip("\n") + "\n\n")
...
def _register_path():
    ...
    return os.path.join(here, "docs", "ks3", "statutory-register.md")


def _deleted_runs(existing, rendered):
    """Contiguous runs of lines present on disk and absent from the render.

    This is the whole safety mechanism. Everything `render()` produces is
    derived; anything on disk that the render does not reproduce is content
    the generator is about to destroy, and it cannot know whether that is a
    stale paragraph or somebody's ruling. So it does not guess — it names the
    lines and stops.
    """
    import difflib
    a, b = existing.splitlines(), rendered.splitlines()
    runs = []
    for tag, i1, i2, j1, j2 in difflib.SequenceMatcher(
            None, a, b, autojunk=False).get_opcodes():
        # A `replace` that swaps n lines for n lines is a MODIFICATION — a
        # reworded paragraph, a changed owner cell — and flagging those would
        # make the guard fire on every ordinary generator edit, which is how a
        # guard gets a --force typed at it reflexively and stops meaning
        # anything. Net loss is the signal: a deletion, or a replace that
        # gives back fewer lines than it took.
        if tag == "delete" or (tag == "replace" and (i2 - i1) > (j2 - j1)):
            lines = [l for l in a[i1:i2] if l.strip()]
            if lines:
                runs.append((i1 + 1, lines))
    return runs


def check_only():
    """Verify without writing. This is what every automated run uses."""
    n = verify()
    if self_test():
        return 1
    out = _register_path()
    ...
    try:
        ...
        print("\u274c %s cannot be read (%s) \u2014 the register is missing."
              % (out, exc))
        return 1
    if existing == rendered:
        print("\u2705 verified %d statements; %s matches the module byte for "
              "byte." % (n, out))
        return 0
    ...
    diff = list(difflib.unified_diff(
        ...
        fromfile="on disk", tofile="rendered from ks3_statutory.py"))
    print("\u274c REGISTER DRIFT \u2014 %s does not match what this module "
          "renders.\n" % out)
    print("".join(diff[:200]).rstrip())
    if len(diff) > 200:
        print("   \u2026 %d more diff line(s)." % (len(diff) - 200))
    print("\n   Either the module changed and the register was not "
          "regenerated\n   (`python3 ks3_statutory.py --write`), or the "
          "register was hand-edited.\n   A hand-written ruling belongs in "
          "`RULINGS` in this module, not in the\n   markdown \u2014 that is "
          "the only place a regeneration cannot delete it.")
    return 1


def write(force=False):
    """Regenerate the register, refusing to delete content it cannot reproduce."""
    n = verify()
    out = _register_path()
    rendered = render()
    existing = None
    ...
    if existing == rendered:
        print("verified %d statements; %s already matches \u2014 nothing "
              "written." % (n, out))
        return 0

    if existing is not None:
        runs = _deleted_runs(existing, rendered)
        if runs and not force:
            print("\u274c REFUSING TO WRITE \u2014 this run would delete %d "
                  "block(s) of content\n   from %s that `render()` does not "
                  "reproduce:\n" % (len(runs), out))
            for start, lines in runs:
                ...
                for l in lines[:12]:
                    print("      %s" % l)
                if len(lines) > 12:
                    print("      \u2026 %d more line(s)" % (len(lines) - 12))
                print()
            print("   This is exactly how MRB-232's KS3.B.NUT.02 ruling was "
                  "destroyed by a run\n   that reported PASS. If any of the "
                  "above is a ruling, lift it into `RULINGS`\n   in "
                  "ks3_statutory.py so regeneration reproduces it. If all of "
                  "it is genuinely\n   stale generated prose, re-run with "
                  "--force and say so in the commit.")
            return 1
...
def self_test():
    """Mutation-test the two mechanisms this module now depends on.

    ⊕ MRB-282. `--check-only` runs this every time, on purpose. A self-test
    that has to be remembered by name is the same shape as the orphan gate
    `gate_registry.py` exists to prevent: it works, and one day nobody runs it.

    It touches no file. Both mutations are performed on rendered STRINGS.

    1. THE DESTRUCTION. Render with `RULINGS` emptied — which is exactly the
       output the old generator produced — and ask the write guard whether it
       would notice the loss. If it would not, the guard is decoration.
    2. THE DRIFT. Corrupt one statement row and ask the comparison whether it
       notices. If it would not, `--check-only` is decoration.
    """
    global RULINGS
    problems = []
    ...
    if not RULINGS:
        problems.append(
            "RULINGS is empty, so mutation 1 proves nothing. If every ruling "
            "has genuinely been retired, delete this self-test deliberately "
        ...
        runs = _deleted_runs(with_rulings, without)
        lost = "\n".join(l for _s, ls in runs for l in ls)
        for ruled in keep:
            marker = ruled["block"].strip().splitlines()[0]
            if marker not in with_rulings:
                problems.append("render() does not emit the ruling block for %s"
                                % ruled["id"])
            if marker not in lost:
                problems.append(
                    "MUTATION 1 SURVIVED: dropping %s's ruling from RULINGS "
                    "changes the register, and the write guard does not report "
                    "it as lost content. A regeneration would delete it "
                    "silently \u2014 which is the MRB-232 defect, back."
                    % ruled["id"])
            owner = ruled.get("owner")
            if owner and owner not in with_rulings:
                problems.append(
                    "render() does not emit the overridden owner cell for %s"
                    % ruled["id"])

    # ── mutation 2 · a real register drift must be seen ─────────────────
    rendered = render()
    first = next(iter(statements()))
    corrupt = rendered.replace("| `%s` |" % first["id"],
                               "| `%s` |" % first["id"], 1)
    corrupt = rendered.replace(first["text"], first["text"] + " AND SOMETHING "
                               "NOBODY WROTE", 1)
    if corrupt == rendered:
        problems.append("mutation 2 could not corrupt the register text")
    elif not _differs(corrupt, rendered):
        problems.append(
            "MUTATION 2 SURVIVED: a statement row was changed and the "
            "comparison reports the register as matching.")

    if problems:
        print("\u274c ks3_statutory self-test: %d problem(s)" % len(problems))
        for p in problems:
            print("   \u00b7 %s" % p)
        return 1
    return 0
...
def _differs(existing, rendered):
...
def main(argv=None):
    ...
    if "--write" in argv:
        return write(force="--force" in argv)
    if "--force" in argv:
        print("--force only means anything with --write.")
        return 2
    unknown = [a for a in argv if a not in ("--check-only",)]
    if unknown:
        print("unknown argument(s): %s\n\n"
              "    python3 ks3_statutory.py [--check-only]   verify, write "
              "nothing (the default)\n"
              "    python3 ks3_statutory.py --write          regenerate the "
              "register\n"
              "    python3 ks3_statutory.py --write --force  regenerate even "
              "if content is lost"
              % ", ".join(unknown))
        return 2
    return check_only()
