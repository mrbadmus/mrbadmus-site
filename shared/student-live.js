...
(function () {
  ...
  var DEPS = [
    ...
    /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19. The KS4
       analogue: `slug: "topic"`, generated by `build_student_port.py`'s
       `ks4_lesson_index()` from `ks4_data.classify()`. See `ks4TopicHref`
       above for why this alone is not enough to build a URL (subject and the
       class's own pathway/tier are also needed, and neither belongs in a
       curriculum-wide index). Same position in the list, same reasoning:
       nothing above it needs it. */
  ...
  /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19. A KS4 subtopic
     slug → its lesson page, or "" if any one of the four things it takes is
     missing. Unlike `lessonHref`, a KS4 URL is not a function of the slug
     alone — `combined|triple/{tier}/{subject}/{topic}/{subtopic}.html` also
     needs the SUBJECT (on the bank row itself) and the TOPIC (curriculum
     data, not bank data — `window.MRB_KS4_TOPICS`, generated by
     `build_student_port.py`'s `ks4_lesson_index()` from `ks4_data.classify()`
     and checked against the built tree, exactly as `ks3-lesson-urls.js` is)
     and the CLASS's own pathway and tier, which decide which of the site's
     several copies of that subtopic page this particular class's lesson
     actually was.

     Empty on any missing piece, same discipline as `lessonHref`: a link that
     404s is worse than a button that is not offered, and the caller checks. */
  function ks4TopicHref(slug, subject, pathway, tier) {
    if (!slug || !subject || !pathway || !tier) { return ""; }
    var topic = window.MRB_KS4_TOPICS && window.MRB_KS4_TOPICS[slug];
    if (!topic) { return ""; }
    return "/" + pathway + "/" + tier + "/" + subject + "/" + topic + "/" + slug + ".html";
  }
  ...
  async function buildClass(sb, user, klass, token) {
    ...
    var coveredSlugs = {};
    var bySlug = {};
    /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19. `ref → {subtopic_slug, subject}`
       for a KS4 bank row, read alongside the two KS3 corpora below. KS4 refs
       and KS3 refs are two disjoint id namespaces (`ks4-…` / the KS3 tables'
       own ids), so a ref answers at most one of the three queries and this map
       can never disagree with `bySlug`. See `ks4TopicHref` below for why the
       TOPIC still has to come from a generated index while the subject does
       not: `ks4_assignment_bank` carries `subject` on the row itself, but no
       `topic` column — a subtopic's topic is curriculum data
       (`ks4_data.classify()`), not bank data. */
    var ks4BySlug = {};
      ...
      if (ids.length) {
        ...
        if (refs.length) {
          /* ⊕ 23 Aug 2026 — the two corpora are read TOGETHER. Both take the
             ...
             ⊕ 25 Sep 2026 (stream K) — a THIRD read joins the same
             `Promise.all`, for the same `refs`, into the separate `ks4BySlug`
             map declared above rather than into this one — MRB-332's whole
             reason to exist is that a KS4 subtopic slug can be
             byte-identical to a KS3 lesson slug, so folding the two id→slug
             facts into one map would reintroduce, client-side, exactly the
             collision the two tables were split to end server-side. */
          var pair = await Promise.all([
            ...
            withDbDeadline(WARM_MS, sb.from("ks4_assignment_bank")
              .select("id, subtopic_slug, subject").in("id", refs))
          ]);
          ...
          (pair[2].data || []).forEach(function (r) {
            ks4BySlug[r.id] = { slug: r.subtopic_slug, subject: r.subject };
          });
        }
        ...
        (aq.data || []).forEach(function (r) {
          ...
          /* ⊕ RULED 25 Sep 2026 (stream K) — TEST 19. A KS4 ref resolves through
             `ks4BySlug` and its own href function; everything else keeps the
             KS3 resolution exactly as it was. */
          var ks4 = ks4BySlug[r.source_ref];
          var slug = ks4 ? ks4.slug : (bySlug[r.source_ref] || slugFromRef(r.source_ref));
          ...
          /* ⊕ RULED 25 Sep 2026 (stream K) — TEST 19. `ks4TopicHref` needs the
             ...
             than guess a tier the class was never assigned. */
          var href = ks4 ? ks4TopicHref(slug, ks4.subject, klass.science_pathway, klass.tier)
                         : lessonHref(slug);
