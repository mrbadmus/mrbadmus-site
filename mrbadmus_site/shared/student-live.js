...
(function () {
  ...
  var DEPS = [
    ...
    /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19, revised the
       same night on the coordinator's instruction. The KS4 analogue:
       `slug: [{subject, topic}, …]`, generated by `build_student_port.py`'s
       `ks4_lesson_index()` from `ks4_data.classify()` — a LIST because a
       KS4 question's subject can no longer be read off a database row
       (`pool_ownership.py` forbids this file naming the KS4 assignment pool
       at all) and has to come from curriculum data instead. See
       `ks4TopicHref` above for how the right entry is picked and why the
       class's own pathway/tier are also needed. Same position in the list,
       same reasoning: nothing above it needs it. */
    "/shared/ks4-lesson-urls.js",
  ...
  /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19, revised the
     same night on the coordinator's instruction: `pool_ownership.py`'s check
     6 is a FLAT PROHIBITION on this file naming the KS4 assignment pool's
     table at all — "the KS4 assignment pool is served by backend
     composition only, and a page that reads it is a page serving from a
     pool it does not own" — so the subject can no longer come off a row of
     it. It comes off
     `window.MRB_KS4_TOPICS`, generated by `build_student_port.py`'s
     `ks4_lesson_index()` from `ks4_data.classify()` (curriculum data, never
     a database read), keyed by slug to a LIST of `{subject, topic}` — a list
     because `classify()` only asserts a slug is unique WITHIN one export; a
     future one that broke that assertion would otherwise be silently
     resolved to whichever entry happened to load first.

     `hint` is the calling assignment's own subject name (lower-cased,
     read off `cards`' `subject_name` — already joined for every card by
     `shared/student-data.js`, so this needs no read of its own either), used
     to pick the right entry when the list holds more than one. With no hint,
     or a hint that matches none of the candidates, or no candidates at all,
     this returns "" — a link that 404s is worse than a button that is not
     offered, and the caller checks. */
  function ks4TopicHref(slug, hint, pathway, tier) {
    if (!slug || !pathway || !tier) { return ""; }
    var entries = (window.MRB_KS4_TOPICS && window.MRB_KS4_TOPICS[slug]) || [];
    var entry = null;
    if (entries.length === 1) {
      entry = entries[0];
    } else if (entries.length > 1 && hint) {
      var matches = entries.filter(function (e) { return e.subject === hint; });
      if (matches.length === 1) { entry = matches[0]; }
    }
    if (!entry) { return ""; }
    return "/" + pathway + "/" + tier + "/" + entry.subject + "/" + entry.topic
         + "/" + slug + ".html";
  }

  /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19, same revision.
     A KS4 bank id's own SHAPE names its subtopic, with no table read needed
     at all: `ks4-<subtopic-slug>-[esh]NN` — `ks4-changes-of-state-h09`,
     `ks4-circuit-symbols-e04` — the trailing letter is the band (easier/
     standard/harder) and the two digits are its position in that band.
     Stripping the `ks4-` prefix and the `-[esh]\d+` suffix leaves exactly the
     subtopic slug `ks4_data.classify()` also uses, with nothing looked up
     anywhere to get it. `null` on anything that is not this shape — a ref
     this build does not recognise is not assumed to be a KS4 one. */
  function ks4SlugFromRef(ref) {
    var m = /^ks4-(.+)-[esh]\d+$/.exec(String(ref || ""));
    return m ? m[1] : null;
  }
  ...
  async function buildClass(sb, user, klass, token) {
    ...
    var lessonsFor = {};
    /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 27. Names only,
       one array of slugs per assignment id, filled below alongside
       `lessonsFor` — see the note there for why the two are not the same
       map. */
    var lessonNamesFor = {};
    ...
    var bySlug = {};
    /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 19, revised the
       same night. `pool_ownership.py` check 6 is a FLAT PROHIBITION on this
       file naming the KS4 assignment pool's table at all — a first draft of
       this ruling read that table for `{subtopic_slug, subject}` and the
       push was refused for it. There is no read here of any kind for a KS4
       ref: its
       subtopic slug comes off the ref's OWN SHAPE (`ks4SlugFromRef`, above —
       `ks4-<subtopic-slug>-[esh]NN`) and its subject comes off the
       assignment's own `subject_name`, already on every `cards` row via
       `shared/student-data.js`'s join — never a database read, and never a
       question's content. */
    /* ⚠️ `detail.assignmentsDueNow`/`…ComingUp`/`…Done` DIRECTLY, NOT `cards`
       — `cards` (the same concatenation, a few screens down) is declared
       AFTER this point in the function, for reasons its own comment gives,
       and this read needs to run before the per-question loop just below
       resolves any href. Same three arrays, read one screen early. */
    var subjectForAssignment = {};
    (detail && detail.assignmentsDueNow
      ? detail.assignmentsDueNow.concat(detail.assignmentsComingUp || [],
                                        detail.assignmentsDone || [])
      : []).forEach(function (c) {
      if (c.subject_name) {
        subjectForAssignment[c.id] = String(c.subject_name).toLowerCase();
      }
    });
      ...
      if (ids.length) {
        ...
        if (refs.length) {
          /* ⊕ 23 Aug 2026 — the two corpora are read TOGETHER. Both take the
             ...
             precedence is the precedence it has always had. A KS4 ref never
             reaches either table — it answers neither `.in("id", refs)` nor
             `.in("question_ref", refs)`, being neither a KS3 bank id nor a
             KS3 ladder ref — so it is resolved separately, below, off its own
             shape, and this pair stays exactly the two reads it always was. */
        ...
        (aq.data || []).forEach(function (r) {
          ...
          /* ⊕ RULED 25 Sep 2026 (stream K) — TEST 19. A KS4 ref's slug comes
             off its own shape, no table involved; everything else keeps the
          ...
          /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 27. A NAME
             ...
             vs `coveredSlugs` above) and "DRAWS ON" only ever prints a name,
             never a link. Filled before the `if (!href) return` below, so a
             lesson this build cannot navigate to still gets read out loud. */
          if (slug) {
            var names = lessonNamesFor[r.assignment_id] ||
                        (lessonNamesFor[r.assignment_id] = []);
            if (names.indexOf(slug) < 0) { names.push(slug); }
          }
          /* ⊕ RULED 25 Sep 2026 (stream K) — TEST 19. `ks4TopicHref` needs the
             CLASS's own pathway and tier — `klass.science_pathway`/`klass.tier`,
             already read for every class by `shared/student-data.js` and
             already in scope here (this whole block runs inside `buildClass`,
             which takes `klass` as an argument). A class with either NULL
             (an unfollowed naming convention — CLAUDE.md's class-naming
             note) cannot resolve one, and `ks4TopicHref` returns "" rather
             than guess a tier the class was never assigned. The subject hint
             is this question's OWN assignment's subject, off
             `subjectForAssignment` above — not the class's, because a
             separate-sciences class's `science_subject` names the class, not
             necessarily the subject of every piece of work on it. */
          var href = ks4Slug
            ? ks4TopicHref(ks4Slug, subjectForAssignment[r.assignment_id],
                           klass.science_pathway, klass.tier)
            : lessonHref(slug);
    ...
    if (current && current.assignment) {
      ...
      if (open.length) {
        ...
        benchWork = {
          ...
          title: pick.title || "",
          /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 27.
             `week_work` carries no `created_at`, and the note this replaces
             said the SET row should stay empty rather than guess at one — but
             `release_at` IS on every `week_work` row (the very field the
             filter three lines above this object already tests), and "when
             this was released" is not a guess, it is the honest answer to
             "when was this set". An empty label after "SET" read as a broken
             page, not as a deliberate omission; `pick.release_at` is exactly
             as real as `pick.due_at` two lines below, which this file has
             always shown without hesitation. */
          setAt: pick.release_at || null,
    ...
    var lessonDefs = [];
    var seen = {};
    /* ⊕ RULED 25 Sep 2026 (experience run, stream K) — TEST 27. `current.
       questions` is populated only for the AUTO-composed assignment
       (`current.assignment`); a bench resolved from `week_work` — the only
       shape a class with automatic weekly work off ever has — carries none,
       so this list, and the docket's "DRAWS ON" row it feeds (below), were
       blank for every teacher-set piece of work. `lessonNamesFor[currentId]`
       is the same source_ref → lesson_slug walk, keyed by assignment id
       rather than by "the current auto assignment", so it answers the
       identical question for a teacher-set bench too. Used only when
       `current.questions` has nothing — the auto path is byte-for-byte
       unchanged. */
