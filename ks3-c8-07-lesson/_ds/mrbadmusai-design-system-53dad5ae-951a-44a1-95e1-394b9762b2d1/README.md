...
- **3D Studio (`--st-*`, in `tokens/src-styles-tokens.css`)** — use for studio screens. Cream world: `--st-ground` #FBF3E6, `--st-paper` #FFFDF8, `--st-ink` #1A140E, `--st-muted`, `--st-caption`, `--st-rule`. Accent: `--st-accent` #E4572E for large/graphic use ONLY; `--st-accent-text` #A63A18 is the only orange at body/small sizes. Dark room: `--st-room` #15110C, `--st-room-panel`, `--st-room-text`, `--st-ember` (accent text on dark). Radii `--st-r-stage`/`--st-r-card`, shadows `--st-shadow-frame`.
...
216 CSS custom properties from mrbadmus-3d-studio. Names are
...
- **other** (187): `--st-display`, `--st-ui`, `--st-mono`, …
