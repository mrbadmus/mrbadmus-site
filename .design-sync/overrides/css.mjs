// forked from design-sync lib/css.mjs — tokensGlob array-of-paths support:
// this repo's token files live at the site root (shared/tokens.css,
// shared/ks3.css), outside the app package, and stock copyTokens only walks
// a single package-rooted glob. Array entries are package-relative file
// paths (../ allowed, lexically resolved through the node_modules self-link)
// copied under sanitized names so two files named tokens.css cannot collide.
...
export function copyTokens({ tokensPkg, tokensGlob, nodeModules, out }) {
  ...
  if (Array.isArray(tokensGlob)) {
    for (const rel of tokensGlob) {
      const src = resolve(tdir, rel);
      if (!existsSync(src)) {
        console.error(`  ! tokensGlob entry not found: ${rel} (from ${tdir})`);
        continue;
      }
      const name = rel.replace(/^(\.\.\/)+/, '').replace(/[\/]+/g, '-');
      mkdirSync(join(out, 'tokens'), { recursive: true });
      cpSync(src, join(out, 'tokens', name));
      tokenFiles.push(name);
    }
    console.error(`  tokens: ${tokenFiles.length} files from ${tokensPkg}@${tjson.version} (path-list mode)`);
    return tokenFiles;
  }
  if (tokensGlob) {
